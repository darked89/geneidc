...
void PrintSite(site *s, int type,
    ...
    if (type == ACC) {
        acc_context = ACCEPTOR_CONTEXT - p->offset + 3;
    }                                                              /*The 3 is to make sure the full length of the branch profile is included*/
    ...
    if (GFF3) {
        if (type == ACC) {
            sprintf(tmpstr, ";acceptor_profile_score=%1.2f", s->ScoreAccProfile);
            strcat(attribute, tmpstr);

            if (PPT) {
                sprintf(tmpstr, ";ppt_score=%1.2f;ppt_pos=%i", s->ScorePPT, s->PositionPPT);
                strcat(attribute, tmpstr);
            }

            if ((BP && s->class == U2) || (s->class != U2)) {
                sprintf(tmpstr, ";bp_score=%1.2f;bp_pos=%i", s->ScoreBP, s->PositionBP);
                strcat(attribute, tmpstr);
            }
        }

        sprintf(tmpstr, ";type=%s;subtype=%s", s->type, s->subtype);
        strcat(attribute, tmpstr);
