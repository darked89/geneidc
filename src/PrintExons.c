...
/*   char saux2[MAXTYPE]; */
    ...
    if (GFF3) {
        if (!e->evidence) {
            ...
            if (UTR) {
                sprintf(tmpstr, ";rpkm=%1.2f", ((e->R) / ((p2 - p1 + 1) / kb)) / MRM);
                strcat(attribute, tmpstr);
            }
        }
...
void PrintExons(exonGFF  *e,
    ...
    switch (type) {
        ...
        case ORF:
            ...
            break;
        case UTRFIRST:
            strcpy(Type, sUTRFIRST);
            break;
        case UTRFIRSTHALF:
            strcpy(Type, sUTRFIRSTHALF);
            break;
        case UTRINTERNAL:
            strcpy(Type, sUTRINTERNAL);
            break;
        case UTR5INTERNALHALF:
            strcpy(Type, sUTR5INTERNALHALF);
            break;
        case UTR3INTERNALHALF:
            strcpy(Type, sUTR3INTERNALHALF);
            break;
        case UTRTERMINALHALF:
            strcpy(Type, sUTRTERMINALHALF);
            break;
        case UTRTERMINAL:
            strcpy(Type, sUTRTERMINAL);
            break;
        default:
            strcpy(Type, sEXON);
            strand = 'x';
        ...
        if (!e->evidence) {
            if (e->Strand == cFORWARD) {
                ...
                if (!strcmp(e->Type, sUTRFIRST)) {
                    sprintf(tmpstr, ";donor_score=%1.2f", e->Donor->Score);
                    strcat(attribute, tmpstr);
                }
                ...
                if (!strcmp(e->Type, sUTRFIRST)) {
                    sprintf(tmpstr, ";donor_score=%1.2f", e->Acceptor->Score);
                    strcat(attribute, tmpstr);
                }
...
void PrintGIntron(exonGFF *d,
    ...
    if (evidence) {
        ...
        if (UTR) {
            sprintf(tmpstr, ";homology_score=%1.2f;rpj=%1.0f", score, ((score / EvidenceFactor) - EvidenceEW));
            strcat(attribute, tmpstr);
...
void PrintGExon(  exonGFF  *a,
    ...
    if (GFF3) {
        ...
        if (!e->evidence) {
            if (e->Strand == cFORWARD) {
                if (!strcmp(e->Type, sFIRST)) {
                    sprintf(tmpstr, ";donor_score=%1.2f", a->Donor->Score);
                    strcat(attribute, tmpstr);
                }

                if (!strcmp(e->Type, sINTERNAL)) {
                    sprintf(tmpstr, ";acceptor_score=%1.2f;donor_score=%1.2f", d->Acceptor->Score, a->Donor->Score);
                    strcat(attribute, tmpstr);

                }

                if (!strcmp(e->Type, sTERMINAL)) {
                    sprintf(tmpstr, ";acceptor_score=%1.2f", d->Acceptor->Score);
                    strcat(attribute, tmpstr);
                }

                if (!strcmp(e->Type, sSINGLE)) {
                    sprintf(tmpstr, ";start_score=%1.2f", d->Acceptor->Score);
                    strcat(attribute, tmpstr);
                }
                ...
                if (!strcmp(e->Type, sFIRST)) {
                    sprintf(tmpstr, ";donor_score=%1.2f", a->Acceptor->Score);
                    strcat(attribute, tmpstr);
                }

                if (!strcmp(e->Type, sINTERNAL)) {
                    sprintf(tmpstr, ";acceptor_score=%1.2f;donor_score=%1.2f", d->Donor->Score, a->Acceptor->Score);
                    strcat(attribute, tmpstr);
                }

                if (!strcmp(e->Type, sTERMINAL)) {
                    sprintf(tmpstr, ";acceptor_score=%1.2f", d->Donor->Score);
                    strcat(attribute, tmpstr);
                }

                if (!strcmp(e->Type, sSINGLE)) {
                    sprintf(tmpstr, ";start_score=%1.2f", d->Donor->Score);
                    strcat(attribute, tmpstr);
                }
            ...
            if (SRP) {
                sprintf(tmpstr, ";homology_score=%1.2f", homology_score);
                strcat(attribute, tmpstr);
            }

            if (UTR) {
                sprintf(tmpstr, ";rpkm=%1.2f", rpkm);
                strcat(attribute, tmpstr);
            }
        }
